...
var searchChar = function(e) {
  ...
  $.ajax({
    dataType: 'json',
    type: 'POST',
    url: '/search_true_id_villain',
    data: obj,
    success: function(data) {
      var results = data.results;
      $('.search_results').html('');
      for (var i = 0; i < results.length; i++) {
        var str = '<li>' + results[i].first_name + ' ' + results[i].last_name + '</li>';
...
var render = function() {
   renderTable('/true_id', '#true_id');
   renderTable('/villain', '#villain');
   renderTable('/special_abilities', '#special_abilities');
   renderTable('/status', '#status');
   renderTable('/true_id_villain', '#true_id_villain');
   renderTable('true_id_status', '#true_id_status');
   renderTable('/true_id_abilities', '#true_id_abilities');
   renderTable('/villain_abilities', '#villain_abilities');
    renderTable('/villain_status', '#villain_status');
